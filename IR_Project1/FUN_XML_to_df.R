...
for (i in 1:length(xml.all)) {
  ...
  if (length(Abstract)==0) {
    ...
    if (length(Abstract)==1) {
      Abstract.All <- list(Abstract[["AbstractText"]]) 
    }else {
      
      Abstract.All <- Abstract[[1]][["text"]]
      for (j in 2:(length(Abstract)-1)) {
        Abstract.All <- paste0(Abstract.All," ", Abstract[[j]][["text"]]) 
      }
    }
  ...
  #  XML.df[i,7] <- length(gregexpr('[[:alnum:]][ ]', Abstract.All)[[1]])
  ...
  #  XML.df[i,8] <- length(gregexpr('[[:alnum:] ][.!?]', Abstract.All)[[1]])
    ...
    rm(Abstract.All, Abstract.All_df, Abstract.All_df.Word)
  }
}
return(XML.df)
}